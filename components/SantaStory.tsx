...
const holidayDispatches = [
  "Santa is officially on holiday. The elves have been told not to call unless it's an emergency.",
  "He's doing 'research' on gift ideas. At least that's what he told Mrs Claus.",
  "The reindeer have been sent to a spa. Rudolph's getting his nose polished.",
  "Santa's out-of-office is on. Auto-reply: Ho Ho Ho, I'm on holiday!",
  "Workshop's closed for summer. The elves are learning to surf.",
  "Santa said he'd only check his naughty list once a week. It's been three weeks.",
  "Mrs Claus finally got him out of the workshop. He lasted 12 minutes before checking his list.",
  "The toy-testing department is running unsupervised. What could go wrong?",
  "Santa's been spotted trying to use a self-checkout. It did not go well.",
  "He promised no work talk on holiday. He's already mentioned chimneys four times.",
  "The elves have started a group chat without Santa. It's mostly memes about his driving.",
  "Santa tried to pack light. He still brought three spare hats and a backup beard.",
];
...
export default function SantaStory({ effectiveTime, holiday }: SantaStoryProps) {
  ...
  const [dispatchIndex, setDispatchIndex] = useState(() =>
    Math.floor(Math.random() * holidayDispatches.length)
  );

  useEffect(() => {
    if (!onHoliday) return;
    const interval = setInterval(() => {
      setDispatchIndex((prev) => (prev + 1) % holidayDispatches.length);
    }, 8000);
  ...
  if (onHoliday && holiday) {
    story = holidayDispatches[dispatchIndex];
    label = "Holiday Dispatch";
  } else {
    switch (data.state) {
      case "OFF_SEASON":
        story =
